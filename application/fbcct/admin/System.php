...
class System extends Admin
{
    ...
    public function index()
    {
        ...
        $order = $this->getOrder();
// 读取用户数据
        $data_list = Balance::order($order)->select();
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = Balance::where("id", $id)->setField($field, $value);
        if (false !== $result) {
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
