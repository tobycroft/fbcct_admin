 ...
 */
class TransferOut extends Admin
{
    /**
     * 用户首页
     * @return mixed
    ...
    public function index()
    {
        ...
        $btn = [
            ...
            'href' => url('yes', ['ids' => '__id__'])
        ];

        $btn1 = [
            'title' => '失败',
            'icon' => 'fa fa-fw fa-ban',
            'href' => url('no', ['ids' => '__id__'])
        ];
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (TransferRecordModel::update($data)) {
                $this->success('编辑成功');
            } else {
                $this->error('编辑失败');
            }
        ...
        // 获取数据
        $info = TransferRecordModel::where('id', $id)->find();
    ...
    public function quickEdit($record = [])
    {
        ...
        $result = TransferRecordModel::where("id", $id)->setField($field, $value);
        if (false !== $result) {
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
