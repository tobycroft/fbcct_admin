...
use app\fbcct\model\TrendModel;
...
class Trend extends Admin
{
    ...
    public function index()
    {
        ...
        // 读取用户数据
        $data_list = TrendModel::where($map)->order($order)->paginate();
    ...
    public function edit($id = null)
    {
        ...
        if (session('user_auth.role') != 1) {
            ...
            $user_list = TrendModel::where('role', 'in', $role_list)->column('id');
        ...
        // 获取数据
        $info = TrendModel::where('id', $id)->find();
    ...
    public function setStatus($type = '', $record = [])
    {
        ...
        switch ($type) {
            case 'enable':
                if (false === TrendModel::where('id', 'in', $ids)->setField('status', 1)) {
                    $this->error('启用失败');
                }
                break;
            case 'disable':
                if (false === TrendModel::where('id', 'in', $ids)->setField('status', 0)) {
                    $this->error('禁用失败');
                }
                break;
            case 'delete':
                if (false === TrendModel::where('id', 'in', $ids)->delete()) {
                    $this->error('删除失败');
                }
                break;
            default:
