...
use app\fbcct\model\InvestModeModel;
 ...
 */
class InvestMode extends Admin
{
    /**
     * 用户首页
     * @return mixed
    ...
    public function index()
    {
        ...
        // 读取用户数据
        $data_list = InvestModeModel::where($map)->order($order)->paginate();
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (InvestModeModel::update($data)) {
                $this->success('编辑成功');
            } else {
                $this->error('编辑失败');
            }
        ...
        // 获取数据
        $info = InvestModeModel::where('id', $id)->find();
