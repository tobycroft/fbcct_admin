...
use app\fbcct\model\ComputeRecordModel;
 ...
 */
class ComputeRecord extends Admin {
	/**
	 * 用户首页
	 * @return mixed
	...
	public function index() {
		...
		// 读取用户数据
		$data_list = ComputeRecordModel::where($map)->order($order)->paginate();
...
//            'class' => 'btn btn-xs btn-default ajax-get',
		...
		return ZBuilder::make('table')
			...
			->addColumn('uid', 'uid')
			->addColumn('rid', 'rid')
			->addColumn('orign', '原力池', 'text.edit')
			->addColumn('parallel', '平行池', 'text.edit')
			->addColumn('exchange', '兑换池', 'text.edit')
			->addColumn('out', '已经兑换出去的', 'text.edit')
			->addColumn('date', 'date')
	...
	public function add() {
		...
		if ($this->request->isPost()) {
			...
			if ($user = ComputeRecordModel::create($data)) {
	...
	public function edit($id = null) {
		...
		if (session('user_auth.role') != 1) {
			...
			$user_list = ComputeRecordModel::where('role', 'in', $role_list)->column('id');
		...
		if ($this->request->isPost()) {
			...
			if (ComputeRecordModel::update($data)) {
				$this->success('编辑成功');
			} else {
				$this->error('编辑失败');
			}
		...
		// 获取数据
		$info = ComputeRecordModel::where('id', $id)->find();
	...
	public function setStatus($type = '', $record = []) {
		...
		switch ($type) {
			case 'enable':
				if (false === ComputeRecordModel::where('id', 'in', $ids)->setField('status', 1)) {
					$this->error('启用失败');
				}
				break;
			case 'disable':
				if (false === ComputeRecordModel::where('id', 'in', $ids)->setField('status', 0)) {
					$this->error('禁用失败');
				}
				break;
			case 'delete':
				if (false === ComputeRecordModel::where('id', 'in', $ids)->delete()) {
					$this->error('删除失败');
				}
				break;
			default:
	...
	public function quickEdit($record = []) {
		...
		$result = ComputeRecordModel::where("id", $id)->setField($field, $value);
		if (false !== $result) {
			$this->success('操作成功');
		} else {
			$this->error('操作失败');
