...
class InvestUser extends Admin
{
    ...
    {
        // 获取排序
        $order = $this->getOrder("level desc");
        $map = $this->getMap();
        ...
        return ZBuilder::make('table')
            ...
            ->addColumn('uid', '用户id')
            ->addColumn('freeze_amount', '锁仓')
            ->addColumn('lock_amount', '锁仓额度')
            ->addColumn('on_release', '待释放额度')
            ->addColumn('amount', '分红权额度')
            ->addColumn('level', 'vip等级', 'text.edit')
            ->addColumn('level_amount', '推荐总金额')
            ->addColumn('num', '推荐人数')
