...
use app\fbcct\model\InvestOrderModel;
 ...
 */
class InvestOrder extends Admin
{
    /**
     * 用户首页
     * @return mixed
    ...
    public function index()
    {
        ...
        // 读取用户数据
        $data_list = InvestOrderModel::where($map)->order($order)->paginate();
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if (InvestOrderModel::update($data)) {
                $this->success('编辑成功');
            } else {
                $this->error('编辑失败');
            }
        ...
        // 获取数据
        $info = InvestOrderModel::where('id', $id)->find();
